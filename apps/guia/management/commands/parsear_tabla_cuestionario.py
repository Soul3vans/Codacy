def _parsear_tabla_docx(self, path):
        """
        Parsea las tablas de cuestionario de un archivo DOCX y retorna la estructura para tablas_cuestionario.
        """
        ...
        for tabla in doc.tables:
            for fila in tabla.rows:
                ...
                print("[DEBUG] Tabla detectada:", celdas)
                if not any(celdas):
                    continue
                ...
                if len(set([c for c in celdas if c])) == 1 and len(celdas) >= 3:
                    texto_componente = limpiar_texto(celdas[0])
                    if texto_componente:
                       if componente_actual is None:
                            componente_actual = {
                                "componente": "Componente no identificado",
                                "bloques": []
                            }
        ...
        # Filtrar componentes vacíos o sin bloques
        tablas_filtradas = [c for c in tablas_cuestionario if c.get("componente") and c.get("bloques")]
        self.contenido_procesado = {"tablas_cuestionario": tablas_filtradas}
        self.save()

def _parsear_tabla_pdf(self, path):
        """
        Parsea las tablas de cuestionario de un archivo PDF y retorna la estructura para tablas_cuestionario.
        """
        import PyPDF2
        import re
        def limpiar_texto(texto):
